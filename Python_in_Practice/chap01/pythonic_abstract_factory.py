"""
ノート
古典的なAbstractFactoryの欠点
1, ファクトリーのインスタンスについて実際にインスタンス化する必要がない。つまりクラスメゾットを使用すればよい。
2, DiagramFactoryとSvgDiagramFactoryの内容がほとんど同じであるという点
3, 最上位のクラス空間においてクラス数が多い点。
pythonにおける改善策
1, FactoryクラスにDiagram, Recatangle, Textの三クラスをネストする。
2, DiagramFactoryの子クラスにSvgDiagramFactoryがなる事でmake_diagram以下が必要なくなる
3, クラスメゾットを使用してcls.コンポーネントクラスとしファクトリーのインスタンス化を防ぐ。

locals関数は、場所によっても変数を変えるが、関数内では引数のみの情報を保持する。
しかし、グローバルスコープにおいてはglobals関数と同じ情報を持つ。
...
def main():
    ...
    txt_diagram = create_diagram(DiagramFactory)  # ファクトリーのインスタンス化を行っていない。
...
font-family="sans-serif" font-size="{fontsize}">{text}</text>"""
    ...
    SVG_SCALE = 20
    # 元々のSvgDiagramクラス、ネストしたために名前空間内部で衝突する可能性がなくなったため
